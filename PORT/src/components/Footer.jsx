...
const socialLinks = [
  {
    ...
    iconClassName: 'h-5 w-5 fill-current sm:h-5 sm:w-5 md:h-5 md:w-5 lg:h-6 lg:w-6 xl:h-7 xl:w-7',
    wrapperClassName: 'hover:opacity-80',
    viewBox: '0 0 640 640',
  ...
  },
  {
    ...
    iconClassName: 'h-7 w-7 fill-current sm:h-7 sm:w-7 md:h-7 md:w-7 lg:h-8 lg:w-8 xl:h-9 xl:w-9',
    wrapperClassName: 'hover:opacity-80',
    viewBox: '0 0 640 640',
...
function SocialIcon({ href, label, wrapperClassName, iconClassName, viewBox, path }) {
  return (
    <a aria-label={label} className={wrapperClassName} href={href} rel="noreferrer" target="_blank">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox={viewBox} className={iconClassName} aria-hidden="true">
        <path d={path} />
      </svg>
    </a>
  );
...
function FooterNavItem({ link, onContactOpen }) {
  if (link.to) {
    return (
      <Link className="hover:opacity-80" to={link.to}>
        {link.label}
      </Link>
    );
  }
  ...
  if (link.action === 'contact') {
    return (
      <button className="text-left hover:opacity-80" onClick={onContactOpen} type="button">
        {link.label}
      </button>
    );
  }
    ...
    <footer className="bg-[#ff97b6] px-4 py-8 text-[#f6efe4] sm:px-8 sm:py-9 lg:px-16 lg:py-12" id="footer">
      ...
      <div className="mx-auto grid w-full max-w-6xl grid-cols-1 gap-6 sm:gap-7 md:grid-cols-3 md:items-center lg:gap-8">
        <nav className="font-montserrat-light hidden flex-col gap-2 text-sm sm:flex sm:text-sm md:text-left md:text-[0.85rem] lg:text-[0.95rem] xl:text-base">
          {navLinks.map((link) => (
            <FooterNavItem key={link.label} link={link} onContactOpen={handleContactOpen} />
          ))}
        </nav>
